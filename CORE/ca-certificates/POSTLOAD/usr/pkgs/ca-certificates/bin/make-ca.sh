...
#
#
# 20170912 - Very small changes to adapt it from LFS to Lombix
#
...
PKIDIR="/usr/pkgs/p11-kit/etc/pki"
SSLDIR="/usr/pkgs/openssl/etc"
CERTUTIL="/usr/pkgs/nss/bin/certutil"
KEYTOOL="/opt/jdk/bin/keytool"
OPENSSL="/usr/pkgs/openssl/bin/openssl"
...
TEMPDIR=$(/usr/pkgs/coreutils/bin/mktemp -d)
  ...
  while test -n "${1}" ; do
    case "${1}" in
      ...
      -P | --pkidir)
        ...
        ANCHORDIR="${PKIDIR}/anchors"
        NSSDB="${PKIDIR}/nssdb"
        echo "${@}" | /usr/pkgs/grep/bin/grep -e "-a " -e "--anchordir" \
      ...
      -S | --ssldir)
        ...
        KEYSTORE="${SSLDIR}/java/cacerts"
        LOCALDIR="${SSLDIR}/local"
        echo "${@}" | /usr/pkgs/grep/bin/grep -e "-c " -e "--cafile" \
                           -e "-d " -e "--cadir"  \
      ...
      -a | --anchordir)
        check_arg $1 $2
        ANCHORDIR="${2}"
        echo "${@}" | /usr/pkgs/grep/bin/grep -e "-P " -e "--pkidir" > /dev/null
        if test "${?}" == "0"; then
      ...
      -c | --cafile)
        check_arg $1 $2
        CABUNDLE="${2}"
        echo "${@}" | /usr/pkgs/grep/bin/grep -e "-S " -e "--ssldir" > /dev/null
        if test "${?}" == "0"; then
...
function check_arg(){
  echo "${2}" | /usr/pkgs/grep/bin/grep -v "^-" > /dev/null
  if [ -z "$?" -o ! -n "$2" ]; then
...
VERSION=$(/usr/pkgs/grep/bin/grep CVS_ID "${CERTDATA}" | cut -d " " -f 8)
...
if test "${VERSION}x" == "x"; then
  ...
  if test "${FORCE}" == "1"; then
    ...
    test -f "${CABUNDLE}" &&
    OLDVERSION=$(/usr/pkgs/grep/bin/grep "^VERSION:" "${CABUNDLE}" | cut -d ":" -f 2)
  fi
fi
...
/usr/pkgs/coreutils/bin/mkdir -p "${TEMPDIR}"/{certs,ssl/{certs,java},pki/{nssdb,anchors},work}
/usr/pkgs/coreutils/bin/cp "${CERTDATA}" "${WORKDIR}/certdata.txt"
...
CERTBEGINLIST=`/usr/pkgs/grep/bin/grep -n "^# Certificate" "${WORKDIR}/certdata.txt" | \
                      cut -d ":" -f1`

# Dump individual certs to temp file
for certbegin in ${CERTBEGINLIST}; do
  /usr/pkgs/gawk/bin/awk "NR==$certbegin,/^CKA_TRUST_STEP_UP_APPROVED/" "${WORKDIR}/certdata.txt" \
...
for tempfile in ${TEMPDIR}/certs/*.tmp; do
  ...
  certname="$(/usr/pkgs/grep/bin/grep "^# Certificate" "${tempfile}" | cut -d '"' -f 2)"

  # Determine certificate trust values for SSL/TLS, S/MIME, and Code Signing
  satrust="$(convert_trust `/usr/pkgs/grep/bin/grep '^CKA_TRUST_SERVER_AUTH' ${tempfile} | \
                  cut -d " " -f 3`)"
  smtrust="$(convert_trust `/usr/pkgs/grep/bin/grep '^CKA_TRUST_EMAIL_PROTECTION' ${tempfile} | \
                  cut -d " " -f 3`)"
  cstrust="$(convert_trust `/usr/pkgs/grep/bin/grep '^CKA_TRUST_CODE_SIGNING' ${tempfile} | \
                  cut -d " " -f 3`)"
  ...
  p11label="$(/usr/pkgs/grep/bin/grep -m1 "Issuer" ${tempfile} | /usr/pkgs/grep/bin/grep -o CN=.*$ | \
              /usr/pkgs/coreutils/bin/cut -d ',' -f 1 | /usr/pkgs/sed/bin/sed 's@CN=@@')"
  ...
  echo "${certtxt}" | /usr/pkgs/sed/bin/sed 's@^@#@' >> "${anchorfile}"
  ...
  if test "${satrust}x" == "Cx"; then
    ...
    if test "${WITH_JAVA}" == "1"; then
      ...
      2>&1> /dev/null | \
      /usr/pkgs/sed/bin/sed -e 's@Certificate was a@A@' -e 's@keystore@Java keystore.@'
    fi

    # GnuTLS certificate bundle
    /usr/pkgs/coreutils/bin/cat tempfile.crt >> "${TEMPDIR}/ssl/ca-bundle.crt.tmp"
  ...
  /usr/pkgs/coreutils/bin/rm -f tempfile.crt
...
bundledir=`echo "${CABUNDLE}" | /usr/pkgs/sed/bin/sed "s@/${bundlefile}@@"`
/usr/pkgs/coreutils/bin/install -vdm755 "${DESTDIR}${bundledir}" 2>&1>/dev/null
test -f "${DESTDIR}${CABUNDLE}" && mv "${DESTDIR}${CABUNDLE}" \
                                      "${DESTDIR}${CABUNDLE}.old"
echo "VERSION:${VERSION}" > "${DESTDIR}${CABUNDLE}"
/usr/pkgs/coreutils/bin/cat "${TEMPDIR}/ssl/ca-bundle.crt.tmp" >> "${DESTDIR}${CABUNDLE}" &&
/usr/pkgs/coreutils/bin/rm -f "${DESTDIR}${CABUNDLE}.old"
unset bundlefile bundledir

# Install Java Cacerts
if test "${WITH_JAVA}" == "1"; then
  javafile=`basename "${KEYSTORE}"`
  javadir=`echo "${KEYSTORE}" | /usr/pkgs/sed/bin/sed "s@/${javafile}@@"`
  /usr/pkgs/coreutils/bin/install -vdm755 "${DESTDIR}${javadir}" 2>&1>/dev/null
  test -f "${DESTDIR}${KEYSTORE}" && mv "${DESTDIR}${KEYSTORE}" \
                                        "${DESTDIR}${KEYSTORE}.old"
  /usr/pkgs/coreutils/bin/install -m644 "${TEMPDIR}/ssl/java/cacerts" "${DESTDIR}${KEYSTORE}" &&
  /usr/pkgs/coreutils/bin/rm -f "${DESTDIR}${KEYSTORE}.old"
  unset javafile javadir
fi

# Install NSS Shared DB
if test "${WITH_NSS}" == "1"; then
  /usr/pkgs/sed/bin/sed -e "s@${TEMPDIR}/pki/nssdb@${NSSDB}@"              \
  ...
  test -d "${DESTDIR}${NSSDB}" && mv "${DESTDIR}${NSSDB}" \
                                     "${DESTDIR}${NSSDB}.old"
  /usr/pkgs/coreutils/bin/install -dm755 "${DESTDIR}${NSSDB}" 2>&1>/dev/null
  /usr/pkgs/coreutils/bin/install -m644 "${TEMPDIR}"/pki/nssdb/{cert9.db,key4.db,pkcs11.txt} \
                 "${DESTDIR}${NSSDB}" &&
  /usr/pkgs/coreutils/bin/rm -rf "${DESTDIR}${NSSDB}.old"
fi

# Install anchors in $ANCHORDIR
test -d "${DESTDIR}${ANCHORDIR}" && mv "${DESTDIR}${ANCHORDIR}"\
                                       "${DESTDIR}${ANCHORDIR}.old"
/usr/pkgs/coreutils/bin/install -dm755 "${DESTDIR}${ANCHORDIR}" 2>&1>/dev/null
/usr/pkgs/coreutils/bin/install -m644 "${TEMPDIR}"/pki/anchors/*.pem "${DESTDIR}${ANCHORDIR}" &&
/usr/pkgs/coreutils/bin/rm -rf "${DESTDIR}${ANCHORDIR}.old"

# Install certificates in $CERTDIR
test -d "${DESTDIR}${CERTDIR}" && mv "${DESTDIR}${CERTDIR}" \
                                     "${DESTDIR}${CERTDIR}.old"
/usr/pkgs/coreutils/bin/install -dm755 "${DESTDIR}${CERTDIR}" 2>&1>/dev/null
/usr/pkgs/coreutils/bin/install -m644 "${TEMPDIR}"/ssl/certs/*.pem "${DESTDIR}${CERTDIR}" &&
/usr/pkgs/coreutils/bin/rm -rf "${DESTDIR}${CERTDIR}.old"
...
if test -d "${LOCALDIR}"; then
  for cert in `find "${LOCALDIR}" -name "*.pem"`; do
    ...
    while test "${count}" -lt 10; do
      echo "${subject}" | cut -d "/" -f "${count}" | /usr/pkgs/grep/bin/grep "CN=" >/dev/null \
           && break
      let count++
    done
    certname=$(echo "${subject}" | cut -d "/" -f "${count}" | /usr/pkgs/sed/bin/sed 's@CN=@@')
    ...
    trustlist=$("${OPENSSL}" x509 -in "${cert}" -text -trustout | \
                       /usr/pkgs/grep/bin/grep -A1 "Trusted Uses")
    satrust=""
    smtrust=""
    cstrust=""
    catrust=""
    satrust=$(echo "${trustlist}" | \
              /usr/pkgs/grep/bin/grep "TLS Web Server" 2>&1> /dev/null && echo "C")
    smtrust=$(echo "${trustlist}" | \
              /usr/pkgs/grep/bin/grep "E-mail Protection" 2>&1 >/dev/null && echo "C")
    cstrust=$(echo "${trustlist}" | \
              /usr/pkgs/grep/bin/grep "Code Signing" 2>&1 >/dev/null && echo "C")
    catrust=$(echo "${trustlist}" | \
              /usr/pkgs/grep/bin/grep "Client Auth" 2>&1 >/dev/null && echo "C")

    # Get reject information
    rejectlist=$("${OPENSSL}" x509 -in "${cert}" -text -trustout | \
                     /usr/pkgs/grep/bin/grep -A1 "Rejected Uses")
    if test "${satrust}" == ""; then satrust=$(echo "${rejectlist}" | \
              /usr/pkgs/grep/bin/grep "TLS Web Server" 2>&1> /dev/null && echo "p"); fi
    if test "${smtrust}" == ""; then smtrust=$(echo "${rejectlist}" | \
              /usr/pkgs/grep/bin/grep "E-mail Protection" 2>&1> /dev/null && echo "p"); fi
    if test "${cstrust}" == ""; then cstrust=$(echo "${rejectlist}" | \
              /usr/pkgs/grep/bin/grep "Code Signing" 2>&1> /dev/null && echo "p"); fi
    if test "${catrust}" == ""; then catrust=$(echo "${rejectlist}" | \
              /usr/pkgs/grep/bin/grep "Client Auth" 2>&1> /dev/null && echo "p"); fi


    # Place certificate into trust anchors dir
    p11label="$(/usr/pkgs/grep/bin/grep -m1 "Issuer" ${cert} | /usr/pkgs/grep/bin/grep -o CN=.*$ | \
                cut -d ',' -f 1 | /usr/pkgs/sed/bin/sed 's@CN=@@')"
...
/usr/pkgs/openssl/bin/c_rehash "${DESTDIR}${CERTDIR}" 2>&1>/dev/null
popd > /dev/null
