...
RULES=/usr/services/eudev/etc/udev/rules.d/70-persistent-net.rules 
...
function ignore_if
{
   ...
   if echo $byte2 | /usr/pkgs/grep/bin/grep -q "[2367abef]"; then return 0; fi
...
function comment
{
  ...
  # Default
  driver=$(/usr/pkgs/coreutils/bin/basename $(/usr/pkgs/coreutils/bin/readlink -f $NIC/device/driver/module))
  export COMMENT="net device ${driver}"
}

if ! /usr/pkgs/util-linux/bin/mountpoint -q /sys; then
  msg="/sys mut be mounted"
  usage
fi

if ! /usr/pkgs/util-linux/bin/mountpoint -q /proc; then
...
for NIC in $DEVICES; do
     IF=${NIC##*/}
     if echo $NIC | /usr/pkgs/grep/bin/grep -q '*' ; then continue; fi

     export INTERFACE=${NIC##*/}            # Simple interface name
     export MATCHADDR="$(/usr/pkgs/coreutils/bin/cat $NIC/address)" # Read MAC address

     VENDOR=$(echo $MATCHADDR | /usr/pkgs/coreutils/bin/cut -c-9)
     if ignore_if; then continue; fi

     export MATCHDEVID="$(/usr/pkgs/coreutils/bin/cat $NIC/dev_id)"
     export MATCHIFTYPE="$(/usr/pkgs/coreutils/bin/cat $NIC/type)"  # Read interface type
     comment

     /usr/services/eudev/lib/udev/write_net_rules 
done
